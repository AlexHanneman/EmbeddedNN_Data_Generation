...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
    ...
    if preprocess == 'average':
        ...
        average_window_width = int(math.floor(
            fft_bin_count / feature_bin_count))
        fingerprint_width = int(
            math.ceil(fft_bin_count / average_window_width))
...
def create_conv_model(fingerprint_input, model_settings, is_training):
    ...
    final_fc_bias = tf.compat.v1.get_variable(
        ...
        initializer=tf.compat.v1.zeros_initializer,
        shape=[label_count])
    final_fc = tf.matmul(flattened_second_conv,
...
def create_low_latency_conv_model(fingerprint_input, model_settings,
    ...
    first_fc = tf.matmul(flattened_first_conv,
                         first_fc_weights) + first_fc_bias
    if is_training:
